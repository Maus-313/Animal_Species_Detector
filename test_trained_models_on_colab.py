...
model_0 = tf.keras.models.load_model('/content/drive/My Drive/models/model_0.h5')
model_1 = tf.keras.models.load_model('/content/drive/My Drive/models/model_1.h5')
model_2 = tf.keras.models.load_model('/content/drive/My Drive/models/model_2.h5')
...
def preprocess_image(img_path, img_height=224, img_width=224):
    ...
    if img_height == 229:  # For InceptionResNetV2
        img_array = tf.keras.applications.inception_resnet_v2.preprocess_input(
            img_array)
...
def create_annotated_image(input_path, output_path, species_name, confidence):
    ...
    # Use a font (download a TTF font file if needed, or use a default)
    try:
...
# Path to sample images in Google Drive
sample_dir = "/content/drive/My Drive/sample_images"  # Create this folder and add sample images
